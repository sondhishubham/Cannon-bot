...
public:
    ...
    game()
    {
        ...
        for(int i=0; i<m; i+=2) {
            for (int j = n-3; j < n; j++) {
                ...
                whites.push_back(new Pawn(Pawn::WHITE, i+1, n-1-j));
                board[i][j]=blackCount;
                board[i+1][n-1-j]=whiteCount;
    ...
    int heuristic;
    int whiteAlive=3*m/2,blackAlive=3*m/2;
    ...
    if (enemy == Pawn::WHITE) {
      //destroy townhall of enemy
        if (isSelfTownHallAt(x, y, enemy) && destroyedWhite[x/2]==1) {//if the enemy townHall is present at the next move
            ...
            int num_townhalls = 0;
            for(int a=0; a<m/2; a++) num_townhalls += destroyedWhite[a];
            if(num_townhalls==m/2-1) currH+=WIN;
        ...
        {
            for(int i(0);i<m/2;i++) currH += destroyedWhite[i]*(h_blacks[i][x][y]-h_blacks[i][currX][currY]);
        ...
        if (isSelfTownHallAt(x, y, enemy) && destroyedBlack[x/2]==1) {
            ...
            int sum = 0, xx, yy, townHall = x / 2;
            int num_townhalls = 0;
            for(int a=0; a<m/2; a++) num_townhalls += destroyedBlack[a];
            if(num_townhalls==m/2-1) currH-=WIN;
        ...
        {
            for(int i(0);i<m/2;i++) currH -= destroyedBlack[i]*(h_whites[i][x][y]-h_whites[i][currX][currY]);
        }
    }
    return currH;
}
...
hIndex MinVal(game g, int alpha, int beta, int maxPlies, int numPlies, Pawn::Side p)
{
    ...
    for (int i(0);i<children.size();i++)
    {
        ...
        for(int j(0);i<movesRecordWhite.size();j++){
...
hIndex MaxVal(game g, int alpha, int beta,int maxPlies, int numPlies, Pawn::Side p)
{
    ...
    {
        Pawn po=pawns[children[i][0]];
        for(int j(0);i<movesRecordBlack.size();j++){
...
//
    ...
    if(player==2){
        ...
        while(true){
            ...
            if(g.blackAlive>bAlivebefore){movesRecordBlack.clear();}
            if(g.whiteAlive>wAlivebefore){movesRecordWhite.clear();}
            cerr<<"white played\n";
   		    string m = ( (moves[index][1]==0) ? (" M ") : (" B ") );
		    cout<<"S "<<pawn.getcorX()<<" "<<pawn.getcorY()<<m<<moves[index][2]<<" "<<moves[index][3]<<endl;
