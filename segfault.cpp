    ...
    if (enemy == Pawn::WHITE) {
      //destroy townhall of enemy
        ...
        if (isSelfTownHallAt(x, y, enemy) && destroyedBlack[x/2]==1) {
            ...
            for (int i = 0; i < 4; i++)
                currH -= (destroyedWhite[i] * h_blacks[i][x][y] + 3*SOLDIER_VAL);
        }
...
hIndex MinVal(game g, int alpha, int beta, int maxPlies, int numPlies, Pawn::Side p)
{
    ...
    if(children.size()==0) return make_pair(-1,g.heuristic);
    if (numPlies==1)
    ...
    if(player==2){
       while(true){
            ...
            while(numPlies<=MAX_PLIES && heuristic<WIN/2){
                ...
                // cerr<<"white did maxval with index:"<<index<<" and value:"<<heuristic<<endl;
                numPlies++;
            }
